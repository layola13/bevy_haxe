...
/**
 * Reader for efficiently reading events.
 * 
 * Event readers track their position to avoid re-reading events.
 */
class EventReader<T:Event> {
    ...
    /**
     * Reads all new events since last read.
     */
    public function read():Array<T> {
        var events:Events<T> = cast world.getResource(Events);
        if (events == null) {
            return [];
        }
        
        var result = events.getEventsSince(Std.hashCode(this));
        lastReadIndex = events.count() - 1;
        return result;
    }
    
    /**
     * Reads a specific number of events.
     * @param maxEvents Maximum number of events to read
     */
    public function readMany(maxEvents:Int):Array<T> {
        var events:Events<T> = cast world.getResource(Events);
        if (events == null) {
            return [];
        }
        
        var allNew = events.getEventsSince(Std.hashCode(this));
        lastReadIndex = events.count() - 1;
        
        if (allNew.length <= maxEvents) {
            return allNew;
        }
        
        return allNew.slice(0, maxEvents);
    }
}
...
/**
 * System that updates event buffers.
 */
class EventUpdateSystem<T:Event> implements System {
    private var eventType:Any;
    
    public function new(eventType:Any) {
        this.eventType = eventType;
    }
    
    public function name():String {
        return 'EventUpdateSystem_${Type.getClassName(Type.getClass(eventType))}';
    }
    
    public function run(world:World):Void {
        var events:Events<T> = cast world.getResource(eventType);
        if (events != null) {
            events.update();
        }
    }
    ...
    public function initialize(world:World):Void {
        ...
        if (!world.containsResource(eventType)) {
            var events:Events<T> = cast ResourceExtension.getOrInit(world);
        }
    }
}
